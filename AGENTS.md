...
- 2026.06.27 18:45 修复 `side_branch_rules` 聚合对必需方法 skipped 结果的异常中断：当 `progressive_rules_v1` 等显式规则方法因未产出 final verdict 返回 `skipped=True` 时，聚合保留该结果作为中性信号参与规则计算，并在 metadata 记录 `rule_skipped_methods`；新增回归测试覆盖远端 `missing method result(s): ['progressive_rules_v1']` 场景。
